...
model {
  ...
  for (ii in 1:nTracks){
    ...
    t_ana[ii] ~ normal(t_ana_input[ii],3*dt) T[0,T*dt];
    target += sum(log(f[ii]));
  }
}
generated quantities {
  int sigma[nTracks,T];
  matrix[nTracks,T] log_lik;
  real ll;
  matrix[nTracks,T] D_sim;
  matrix[nTracks,nStates+2] aux;
  matrix[T,2] y_sim[nTracks];
  ll=0;
  for(ii in 1:nTracks){
    log_lik[ii] = rep_vector(0,T)';
    for (t in (T0[ii]+1):T1[ii]) {
      log_lik[ii,t] = log(f[ii][t]);
      sigma[ii,t] = categorical_rng(xi[ii][t]');
      if (t==(T0[ii]+1)){
        y_sim[ii][t] = to_row_vector(normal_rng(x0[ii],1));
        D_sim[ii,t] = myDiff(x0[ii]);
      } else {
        for (i in 1:(nStates+2)){
          if (i<=2) {
            aux[ii,i] = y[ii][t-1,i];
          } else if (i==(sigma[ii,t]+2)) {
            aux[ii,i] = 1.0;
          } else {
            aux[ii,i] = 0.0;
          }
        ...
        if (sigma[ii,t]<5){
//          y_sim[ii][t] = y_sim[ii][t-1] + (dt*odeUpdateMatrix(theta[ii])*auxStates)' + (dt*odeUpdateVector(theta[ii], cos_phi[ii][t]))';
//          y_sim[ii][t] = to_row_vector(normal_rng(y_sim[ii][t],sqrt(dt/tau[ii])));
          y_sim[ii][t] = to_row_vector(normal_rng(y[ii][t-1]' + dt*odeUpdateMatrix(theta[ii])*aux[ii]' + dt*odeUpdateVector(theta[ii], cos_phi[ii][t]), sqrt(dt/tau[ii])));
          D_sim[ii,t] = myDiff(to_vector(y_sim[ii][t]));
        } else {
          if (sigma[ii,t]==5){
//            y_sim[ii][t] = y_sim[ii][t-1] + dt*[v_ana[ii], -v_ana[ii]];
//            y_sim[ii][t] = to_row_vector(normal_rng(y_sim[ii][t], sqrt(dt/tau[ii])));
            y_sim[ii][t] = to_row_vector(normal_rng(y[ii][t-1] + dt*[v_ana[ii], -v_ana[ii]], sqrt(dt/tau[ii])));
            D_sim[ii,t] = myDiff(to_vector(y_sim[ii][t]));
  	} else {
//            y_sim[ii][t] = to_row_vector(normal_rng(y_sim[ii][t-1], sqrt(4*dt/tau[ii])));
            y_sim[ii][t] = to_row_vector(normal_rng(y[ii][t-1], sqrt(4*dt/tau[ii])));
            D_sim[ii,t] = myDiff(to_vector(y_sim[ii][t]));
          }
        }
      }
